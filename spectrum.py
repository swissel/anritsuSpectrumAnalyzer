...
class SpectrumAnalyzer():
	...
	def getCommonInfo(self, run=None, freqband=None, boxno=None,pol=None, weather=None, daynight=None, comments=None):
	...
	def setupSpectrumAnalyzer(self):
		...
		print("Setting minimum sweep time: ", self.sa.query("SENS:SWE:TIME?"), "s")	
		self.sa.write("SENS:SWETIME %3e6f"%self.sweep_time)
		print("....If this seems long, reduce your span (%3.3e MHz) or increase your RBW (%3.3e kHz)/VBW (%3.3e kHz)."%(self.span/1e6, self.rbw/1e3, self.vbw/1e3))
		...
		print("Setting time between spectra to sweeptime ( %3.3e ms)x ntraces (%d) = %3.3e s "%(self.sweep_time/1e3, self.ntraces, self.spec_time))
	...
	def plotSpectrum(self):
		...
		pyp.subplot(2,1,2)
		pyp.plot(self.freq_hz/1e6, self.data - 10.*np.log10(self.head['rbw_hz']/1e6))
		pyp.xlabel("Frequency (MHz)")
	...
	def plotSpectrogram(self):
		...
		spectrogram = np.zeros( (len(datetimestamps), len(self.dataStream[timestamps[0]]['power_dBm'])))
		...
		freqmesh, timemesh = np.meshgrid(self.dataStream[timestamp]['freq_hz']/1e6, tdelta)
		# plot!
		...
		# plot!
		pyp.subplot(2,1,2)
		pyp.pcolormesh(freqmesh,timemesh, spectrogram - 10.*np.log10(self.rbw/1e6))
		pyp.xlabel("Frequency (MHz)")
		pyp.ylabel("Time(s)")
		pyp.title(starttime.strftime(format=datestrf))
		cbar = pyp.colorbar()
...
specAnal.readSpectrogram(nspectra=100)
